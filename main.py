...
def update_us():
    ...
    status = request.form['status']
    id = request.form['id']
    query =UserStory.update(title=title,story_text=story_text,acceptance_criteria=acceptance_criteria,buisness_value=buisness_value,estimation=estimation,status=status).where(UserStory.id == id)
    query.execute()
    return redirect(url_for('show_user_stories'))
